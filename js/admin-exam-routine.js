 ...
 * Version: v3.0 — Session Helper + Image/PDF Support
 * Depends: config.js, supabase.js, auth.js, cloudinary.js, admin-popup.js, session-helper.js
 * Table: exam_routines + departments + courses + batches
 * =========================================================
 */
...
(function () {
    ...
    async function loadAllRoutines() {
        try {
            const { data, error } = await window.FDC_SUPABASE
                .from('exam_routines').select('*').order('created_at', { ascending: false });
        ...
        filteredRoutines = allRoutines.filter(r => {
            ...
            if (search) {
                const text = ((r.title || '') + ' ' + (r.group_name || '') + ' ' +
                    (r.department || '') + ' ' + (r.class_year || '') + ' ' + (r.batch || '') + ' ' + (r.exam_name || '')).toLowerCase();
    ...
    function updateDynamicFields(branch) {
        ...
        if (branch === 'HSC') {
            ...
            HSC_GROUPS.forEach(g => field1.insertAdjacentHTML('beforeend', `<option value="${g}">${g}</option>`));
            HSC_YEARS.forEach(y => field2.insertAdjacentHTML('beforeend', `<option value="${y}">${y}</option>`));
        }
        else if (branch === 'BM') {
            wrap2.style.display = 'block';
            label2.textContent = 'Year';
            BM_YEARS.forEach(y => field2.insertAdjacentHTML('beforeend', `<option value="${y}">${y}</option>`));
        }
            ...
            MASTER.departments.forEach(d => {
                ...
                field1.insertAdjacentHTML('beforeend', `<option value="${escapeHtml(displayName)}">${escapeHtml(displayName)}</option>`);
            });
    ...
    function autoSuggestTitle() {
        ...
        if (branch === 'HSC') {
            ...
            parts.push('Honours');
            if (field1) parts.push(field1);
            if (field2) parts.push('Session ' + field2);
        } else if (branch === 'Degree') {
            parts.push('Degree');
            if (field1) parts.push(field1);
            if (field2) parts.push('Session ' + field2);
        ...
        if (parts.length > 0) titleEl.value = parts.join(' ') + ' Exam Routine';
    }
    ...
    async function saveRoutine() {
        ...
        try {
            ...
            if (selectedImageFile) {
                if (typeof window.FDCUploadImage !== 'function') throw new Error('Upload function not available');
            ...
            let result;
            if (editingId) {
                result = await window.FDC_SUPABASE.from('exam_routines').update(payload).eq('id', editingId).select();
            } else {
                result = await window.FDC_SUPABASE.from('exam_routines').insert([payload]).select();
            }
    ...
    function deleteRoutine(id) {
        ...
        window.fdcPromptInput({
            ...
            subtitle: r.title,
            message: `এই exam routine <strong>চিরতরে</strong> delete হবে।<p style="margin-top:12px;color:#7f1d1d;font-weight:700;">এটি undo করা যাবে না!</p>`,
            ...
            onConfirm: async function () {
                try {
                    const { error } = await window.FDC_SUPABASE.from('exam_routines').delete().eq('id', id);
                    if (error) throw error;
    ...
    async function togglePublish(id) {
        ...
        try {
            const { error } = await window.FDC_SUPABASE.from('exam_routines').update({ is_published: newState }).eq('id', id);
            if (error) throw error;
            window.fdcSuccess(`Routine ${newState ? 'Publish' : 'Unpublish'} হয়েছে!`);
            await loadAllRoutines();
        } catch (e) {
    ...
    async function init() {
        console.log('🚀 Admin Exam Routine v3.0 initializing...');
        attachEvents();
        setupImageUpload();
        ...
        waitForSupabase(async function () {
            ...
            console.log('✅ Admin Exam Routine v3.0 ready');
        });
    }

/**
 * =========================================================
 * FULBARIYA COLLEGE — ADMIN SCOUTS MANAGEMENT
 * Location: js/admin-scouts.js
...
(function () {
    ...
    async function loadAllMembers() {
        try {
            ...
            allMembers = data || [];
            console.log('✅ Loaded scouts members:', allMembers.length);
    ...
    function updateStats() {
        ...
        const leader = allMembers.filter(function (m) { return m.member_type === LEADER_TYPE; }).length;
        const members = allMembers.filter(function (m) { return m.member_type !== LEADER_TYPE; }).length;
        ...
        if (officerEl) officerEl.textContent = leader;
    ...
    function renderLeader() {
        ...
        const leader = allMembers.find(function (m) { return m.member_type === LEADER_TYPE; });

        if (!leader) {
            card.innerHTML = '<div class="pi-empty">' +
                '<i class="fas fa-user-tie"></i>' +
                '<p>Scout Leader যোগ করা হয়নি</p>' +
                '<button class="btn-pi-add" onclick="window.showAddLeader()">' +
                    '<i class="fas fa-plus"></i> Add Scout Leader' +
                '</button>' +
            '</div>';
            return;
        }

        const img = leader.image_url || 'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect fill=%22%23f8f9fc%22 width=%22100%22 height=%22100%22/%3E%3Ctext x=%2250%25%22 y=%2250%25%22 fill=%22%230a1655%22 font-size=%2240%22 text-anchor=%22middle%22 dy=%22.35em%22%3E%F0%9F%91%A4%3C/text%3E%3C/svg%3E';

        let contact = '';
        if (leader.phone) {
            contact += '<span><i class="fas fa-phone"></i> ' + escapeHtml(leader.phone) + '</span>';
        }

        card.innerHTML =
            '<span class="pi-badge">⭐ Scout Leader</span>' +
            '<img src="' + escapeHtml(img) + '" alt="' + escapeHtml(leader.name) + '" class="pi-img" onerror="this.src=\'data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%22100%22 height=%22100%22%3E%3Crect fill=%22%23f8f9fc%22 width=%22100%22 height=%22100%22/%3E%3C/svg%3E\'">' +
            '<div class="pi-info">' +
                '<span class="pi-designation">' + escapeHtml(leader.designation || 'Scout Leader') + '</span>' +
                '<h3>' + escapeHtml(leader.name) + '</h3>' +
                (leader.name_en ? '<div class="pi-name-en">' + escapeHtml(leader.name_en) + '</div>' : '') +
                (contact ? '<div class="pi-contact">' + contact + '</div>' : '') +
            '</div>' +
            '<div class="pi-actions">' +
                '<button class="pi-btn" onclick="window.editMember(\'' + leader.id + '\')" title="Edit">' +
                    '<i class="fas fa-edit"></i>' +
                '</button>' +
                '<button class="pi-btn" onclick="window.deleteMember(\'' + leader.id + '\')" title="Delete">' +
    ...
    function renderMembers() {
        ...
        let filtered = allMembers.filter(function (m) {
            if (currentFilter === 'leader' && m.member_type !== LEADER_TYPE) return false;
            if (currentFilter === 'member' && m.member_type === LEADER_TYPE) return false;
        ...
        filtered.forEach(function (m) {
            ...
            const leaderBadge = m.member_type === LEADER_TYPE
                ? '<span class="mc-badge-officer">Leader</span>'
                : '';
    ...
    function showAddForm(presetType) {
        ...
        if (presetType === 'leader') {
            document.getElementById('memberType').value = 'leader';
            document.getElementById('designation').value = 'Scout Leader';
        } else {
    ...
    window.showAddLeader = function () {
        const existing = allMembers.find(function (m) { return m.member_type === LEADER_TYPE; });
        if (existing) {
            showToast('Scout Leader already exists. Edit the existing one.', 'error');
            return;
        }
        showAddForm('leader');
    };
    ...
    window.deleteMember = async function (id) {
        ...
        try {
            const { error } = await window.FDC_SUPABASE.from(TABLE).delete().eq('id', id);
            if (error) throw error;
    ...
    function bindFormSubmit() {
        ...
        memberForm.addEventListener('submit', async function (e) {
            ...
            if (member_type === LEADER_TYPE) {
                const existingLeader = allMembers.find(function (m) {
                    return m.member_type === LEADER_TYPE && m.id !== id;
                });
                if (existingLeader) {
                    showToast('Scout Leader already exists. Only one allowed.', 'error');
                    return;
                }
            }
            ...
            try {
                ...
                let result;
                if (id) {
                    result = await window.FDC_SUPABASE.from(TABLE).update(memberData).eq('id', id).select();
                    showToast('Member updated!', 'success');
                } else {
                    result = await window.FDC_SUPABASE.from(TABLE).insert([memberData]).select();
    ...
    function init() {
        console.log('🚀 Admin Scouts initialized');
    ...
    console.log('✅ admin-scouts.js loaded');
